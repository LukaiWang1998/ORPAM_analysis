% Modified from '\\10.229.121.108\workspace\YIXIAO\ORPAM
% ANALYSIS\codes\BSCAN_ANALYSIS_20240415.m'. 
clc;
clear;
data_total = 256;
...
aqdate = '20250415_Thaker_EC_38';
filepath = 'C:\Users\Admin\OneDrive - Washington University in St. Louis\ORPAM Sample';
...
[RF_raw_all, RF_Sum_all] = synthetic_aperture_beamforming(raw_data1, info, HF_cancel, DC_cancel, h, N, Nzeropad, N_repeat);
%
RF_DAS_env = RF_Sum_all / max(RF_Sum_all(:));
%
log_DAS_data = dynamic_ranging_log_compression(RF_DAS_env, 45);
...
save(fullfile(SaveFolder, 'spectra_cube_avgAline.mat'), 'spectra_cube_avgAline', '-v7.3');

% Plot by the aspect ratio of the data
fig = figure;
imshow(im_plot',[0.0,0.6],'border','tight');
colormap('hot');
set(gca,'xtick','','ytick','','fontweight','bold');
...
fig = figure;
imshow(im_plot',[0.0,0.2],'border','tight');
colormap('hot');
set(gca, 'xtick', [], 'ytick', [], 'fontweight', 'bold');
axis image;  % Maintain aspect ratio: 1000 pixels tall
